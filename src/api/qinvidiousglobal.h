...
void parseArray(const QJsonValue &val, QList<T> &values)
{
    ...
    std::transform(array.begin(), array.end(), std::back_inserter(values), [](const QJsonValue &val) {
...
{
    const auto array = val.toArray();
    std::transform(array.cbegin(), array.cend(), std::back_inserter(values), [](const QJsonValue &val) {
        return val.toString();
    });
}
